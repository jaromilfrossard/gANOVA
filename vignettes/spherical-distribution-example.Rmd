...
vignette: >
  %\VignetteIndexEntry{Spherical distribution of the random effects: example}
...
We present here some examples to highlight the difference between the `gANOVA()` function and the built-in spherical distribution of `lmer()`. Both function estimate a spherical correlation structure. However the parametrization in using `gANOVA()` increase the parameter space and a wider range of models can be estimated without having estimate at the boundary of the parameter space (variance estimation at 0).
...
Estimating the model using `gANOVA()` reduce the constraint on the parameter space (of the variance of the intercept). The `gANOVA` parametrization produces a non-zero estimate of the variance of the random intercept. It follows that the likelihood is smaller:
